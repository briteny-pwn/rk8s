...
impl Filesystem for OverlayFs {
    ...
    async fn rename2(
        ...
        tracing::debug!("rename2: parent={}, name={:?}, new_parent={}, new_name={:?}, flags={:#x}", 
        ...
        if flags & libc::RENAME_EXCHANGE != 0 {
            ...
            let new_name_str = new_name.to_str().ok_or_else(|| {
                std::io::Error::from_raw_os_error(libc::EINVAL)
            })?;
            ...
            let dest_node = self.lookup_node(req, new_parent, new_name_str).await
                .map_err(|e| rfuse3::Errno::from(e.raw_os_error().unwrap_or(libc::ENOENT)))?;
            
            // Check for whiteouts
            if src_node.whiteout.load(std::sync::atomic::Ordering::Relaxed) || 
               dest_node.whiteout.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            let rename_result = p_layer.rename2(req, p_inode, name, new_p_inode, new_name, flags).await;
            ...
            let _ = std::fs::write("/tmp/atomic_exchange_inline", format!("Inlined: {} <-> {}", name_str, new_name_str));
            
            // Calculate new paths
            let src_new_path = format!("{}/{}", new_pnode.path.read().await, new_name_str);
            let dst_new_path = format!("{}/{}", pnode.path.read().await, name_str);
            tracing::warn!("INLINED: src_new_path={}, dst_new_path={}", src_new_path, dst_new_path);
            ...
            *d_node.parent.lock().await = std::sync::Arc::downgrade(&pnode);
            ...
            if std::sync::Arc::ptr_eq(&pnode, &new_pnode) {
                ...
                tracing::warn!("INLINED: Before swap: first has {} children, second has {} children", 
        ...
        self.do_rename(req, parent, name, new_parent, new_name, flags).await
            .map_err(|e| e.into())
    }
