...
fn test_p0_basic_rename() {
    ...
    // Create source file
    let src = env.create_file(&env.upper_dir, "source.txt", "hello world").unwrap();
...
fn test_p0_rename_overwrites_existing() {
    ...
    let src = env.create_file(&env.upper_dir, "src.txt", "new content").unwrap();
    let dst = env.create_file(&env.upper_dir, "dst.txt", "old content").unwrap();
...
fn test_p0_rename_to_whiteout_position() {
    ...
    env.create_file(&env.lower_dir, "old.txt", "lower content").unwrap();
    
    // Upper layer has a whiteout (marking old.txt as deleted)
    let whiteout = env.create_whiteout(&env.upper_dir, "old.txt").unwrap();
    assert!(env.is_whiteout(&whiteout).unwrap(), "Should be a whiteout file");
    
    // Create source file
    let src = env.create_file(&env.upper_dir, "new.txt", "new content").unwrap();
    ...
    assert!(!env.is_whiteout(&dst).unwrap(), "Destination should NOT be whiteout");
    assert_eq!(env.read_file(&dst).unwrap(), "new content");
}
...
fn test_p0_rename_from_lower_creates_whiteout() {
    ...
    env.create_file(&env.lower_dir, "file.txt", "content").unwrap();
    
    // Copy to upper layer (simulating copy-up)
    let src = env.create_file(&env.upper_dir, "file.txt", "content").unwrap();
...
fn test_p0_hardlink_rename_no_whiteout() {
    ...
    let file = env.create_file(&env.upper_dir, "file.txt", "content").unwrap();
...
fn test_p0_hardlink_last_link_should_create_whiteout() {
    ...
    // File in lower layer
    env.create_file(&env.lower_dir, "file.txt", "lower").unwrap();
    
    // Single file in upper layer (nlinks=1)
    let src = env.create_file(&env.upper_dir, "file.txt", "upper").unwrap();
    let dst = env.upper_dir.join("renamed.txt");
...
fn test_p0_hardlink_nlink_count_consistency() {
    ...
    let file = env.create_file(&env.upper_dir, "original.txt", "data").unwrap();
    let link1 = env.upper_dir.join("link1.txt");
    let link2 = env.upper_dir.join("link2.txt");
...
fn test_p0_rename_noreplace_fails_if_exists() {
    ...
    let src = env.create_file(&env.upper_dir, "src.txt", "source").unwrap();
...
fn test_p0_rename_noreplace_succeeds_if_not_exists() {
    ...
    let src = env.create_file(&env.upper_dir, "src.txt", "content").unwrap();
    let dst = env.upper_dir.join("dst.txt");
    
    let result = rename_with_flags(&src, &dst, libc::RENAME_NOREPLACE);
    
    assert!(result.is_ok(), "Should succeed when destination doesn't exist");
    assert!(!env.file_exists(&src));
    assert!(env.file_exists(&dst));
...
fn test_p0_rename_whiteout_flag() {
    ...
    // File in lower layer
    let lower_file = env.create_file(&env.lower_dir, "file.txt", "lower").unwrap();
    
    // File in upper layer
    let upper_file = env.create_file(&env.upper_dir, "file.txt", "upper").unwrap();
    let dst = env.upper_dir.join("renamed.txt");
    ...
    assert!(env.is_whiteout(&upper_file).unwrap(), "Source should be whiteout");
...
fn test_p0_rename_whiteout_without_lower() {
    ...
    assert!(result.is_ok());
    assert!(env.file_exists(&dst));
    assert!(env.is_whiteout(&src).unwrap(), "Whiteout created even without lower");
}
...
fn test_p1_rename_exchange_two_files() {
    ...
    let file1 = env.create_file(&env.upper_dir, "file1.txt", "content1").unwrap();
    let file2 = env.create_file(&env.upper_dir, "file2.txt", "content2").unwrap();
...
fn test_p1_rename_exchange_fails_if_not_exists() {
    ...
    let file1 = env.create_file(&env.upper_dir, "file1.txt", "content").unwrap();
...
fn test_p1_rename_symlink_itself() {
    ...
    env.create_file(&env.upper_dir, "target.txt", "target content").unwrap();
...
fn test_p1_rename_symlink_target() {
    ...
    let target = env.create_file(&env.upper_dir, "target.txt", "content").unwrap();
    let link = env.upper_dir.join("link");
    env.symlink("target.txt", &link).unwrap();
...
fn test_p1_error_exchange_with_whiteout() {
    ...
    let file1 = env.create_file(&env.upper_dir, "file1.txt", "content").unwrap();
    let file2 = env.create_file(&env.upper_dir, "file2.txt", "content").unwrap();
...
fn test_p1_rename_directory_with_contents() {
    ...
    env.create_file(&src_dir, "subdir/file2.txt", "content2").unwrap();
...
fn test_p2_rename_special_characters() {
    ...
    let src = env.create_file(&env.upper_dir, "file with spaces.txt", "content").unwrap();
...
fn test_p2_rename_unicode_filename() {
    ...
    let src = env.create_file(&env.upper_dir, "文件.txt", "content").unwrap();
